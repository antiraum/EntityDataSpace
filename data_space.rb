...
class DataSpace
  ...
  def delete_entity(id)
    ...
    @db.delete_if { |k, v| k =~ db_key_regex }

    # delete attributes that have this entity as value
    @db.delete_if { |k, v| v == id }
  end
  ...
  def search(query)
    ...
    if query.value == Entity::ANY_VALUE
      ...
      results.push query.value if @db.has_key? str_to_db_key(query.value)
    end
  ...
  class EntityExistsError < StandardError
    ...
    def initialize(id)
      @id = id
      super "entity with identifier '#{@id}' already exists"
    end
  ...
  class AttributeExistsError < StandardError
    ...
    def initialize(id, key)
      @id = id
      @key = key
      super "attribute with name '#{@key}' for entity '#{@id}' already exists"
    end
  ...
  class NoEntityError < StandardError
    ...
    def initialize(id)
      @id = id
      super "entity with identifier '#{@id}' does not exists"
    end
  ...
  class NoAttributeError < StandardError
    ...
    def initialize(id, key)
      @id = id
      @key = key
      super "attribute with name '#{@key}' for entity '#{@id}' does not exists"
    end
  ...
  def entity_complies?(id, conditions)
    ...
    conditions.each { |c|
      ...
      if c.key == Entity::ANY_VALUE # any one attribute has to have the value
        ...
        matched = false
        @db.each_key { |k|
          next if k !~ entity_regex || @db[k] != c.value
          matched = true
          break
        }
  ...
  def build_entity(key, value)
    ...
    @db.each_key { |k|
      ...
      childs.push build_entity(db_key_to_str(k.sub(db_key_regex, "")), @db[k])
    }
